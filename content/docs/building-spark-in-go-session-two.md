...
In this session, we introduce distributed execution to establish process and network boundaries. We introduce a physical scheduler and multiple independent worker processes that communicate with the coordinator over HTTP, like deploying microservices on AWS.
